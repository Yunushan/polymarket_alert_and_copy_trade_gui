...
| Interactive Brokers ForecastTrader / IBKR Prediction Markets (`ibkr_forecasttrader`) | Implemented | Yes | Yes (executions/OHLC history) | Yes | Guarded, off by default | No | Required | IBKR account required | Region/KYC limited |
| ForecastEx (`forecastex`) | Implemented | Yes | Yes (executions/OHLC history) | Yes | Guarded, off by default | No | Required | IBKR account required | Region/KYC limited |
| CME Group Prediction Markets (`cme_prediction_markets`) | Implemented | Yes | Yes (executions/OHLC history) | Yes | Guarded, off by default | No | Required | IBKR account required | Region/KYC limited |
...
| Xmarket (`xmarket`) | Implemented | Yes (positions/orders) | Yes | Yes | Guarded, off by default | No | Required | API credentials required | Jurisdiction varies |
