...
- Follows a tracked wallet’s **BUY** trades (SELL optional, guarded) for a selected market with an official activity feed (Polymarket, Opinion Labs, Manifold, Myriad, Hyperliquid HIP-4, and Predict.fun authenticated account activity)
...
- Reads Omen/Gnosis AMM marginal prices plus public FPMM trades, resolves per-collateral token scales from the subgraph, and derives bounded candles that fail closed on ambiguous same-second prices; also reads Zeitgeist indexer asset prices for alerts
- Supports dry-run paper orders where reliable price data exists; Zeitgeist live submission is fail-closed until a runtime-aware signed-extrinsic decoder can bind every call to the reviewed HybridRouter order

### 11) Additional official adapter support
- Reads Gemini Prediction Markets events/contracts, documented contract orderbooks, implied prices, irregular price-history points, and authenticated account recovery (active/history orders, current/settled positions, and event volume metrics) through official endpoints; history is exposed as flat candles without fabricated OHLCV resampling. The headless CLI exposes these private reads as `markets account <operation>` and the web API exposes `/api/markets/{market_id}/account/{operation}` for Gemini, Kalshi, Limitless, Opinion, and Hyperliquid; all use an explicit operation allow-list and never accept arbitrary authenticated paths. Gemini, Betfair, Kalshi, Limitless, Polymarket, Probable, and Hyperliquid expose separate guarded order-management CLI/API/React surfaces using only cancellation or size-reducing endpoints; Gemini supports single and batch cancellation with a local 20-order cap, Limitless supports single, batch, and market-scoped cancellation, Probable composes fixed signed cancellation paths, Kalshi additionally supports size reduction, and Hyperliquid forwards only complete externally signed cancel/cancel-by-client-id/scheduled-cancel envelopes.
- Reads Myriad, Opinion, Predict.fun, XO, Betfair, and Limitless market data through their documented APIs; Betfair matched account orders are normalized as probability-priced trades, Myriad order-book matches are normalized as public trades, XO public market trades and OHLCV candles are normalized through its HMAC REST feed, and XO account, position, order, private-trade, settlement, and audit reads preserve the documented authenticated payloads. Limitless historical YES prices are normalized as flat candles with complementary NO prices and finalized public market-event fills are normalized as trades. Limitless also exposes HMAC-authenticated portfolio positions, account history, and market-specific user orders as raw lossless payloads when approved token credentials and an optional delegated profile are configured. Xmarket exposes authenticated positions, user orders, market-scoped orders, bounded account-scoped filled-order history, derived candles, and guarded fixed-path batch cancellation through its documented API-key endpoints; order-management batch creation is deliberately unavailable, and it does not claim a public trade tape or copy-trading feed.
...
market-sentinel markets support
market-sentinel markets support --market kalshi
...
market-sentinel markets account portfolio --market myriad_markets --wallet 0x... --limit 25 --trading-model all
...
`unlimited` removes local row/enrichment budgets, not upstream restrictions.
The [public leaderboard API](https://docs.polymarket.com/api-reference/core/get-trader-leaderboard-rankings)
currently permits request offsets through 1,000 and pages through 50 rows.
The scanner never requests beyond that offset and records
`completion_reason=upstream_offset_limit`, with
`source_enumeration_complete=false`, when it reaches the boundary. A repeated
...
Expired entries are evicted, oversized entries are not retained, and cached
values are isolated between callers. These are cache-storage budgets, not total
process-memory limits; active requests, parsed histories and other app state
still consume memory. `--mdd-cache-ttl-seconds 0` bypasses cache reads/writes.
...
- `GET /api/markets/support-matrix` and `GET /api/markets/{market_id}/support` return the canonical support contract used by the CLI and React detail view; each operation is classified as `supported`, `guarded`, `unsupported`, or `blocked`, and verified-blocked rows include their reviewed reason/references.
- Wallet activity and copy previews additionally cover Context V2 filled wallet orders, MetaDAO bounded maker swaps, Omen/Gnosis bounded creator trades, Probable public activity, and Azuro single-selection bettor bets. These feeds are validated, bounded, and simulation-first; no activity poll submits a live order.
- Omen/Gnosis, MetaDAO, Manifold, DFlow, Betfair, Matchbook, SX Bet, and Azuro candle responses are bounded, explicitly marked as derived, and retain source trade/fill/execution/bet ids because those public/account feeds do not expose native candle endpoints. Omen/Gnosis candle derivation rejects same-second trades with different prices because the subgraph does not expose a block/log ordering key; MetaDAO history additionally enforces per-call slot-window, scan-window, streamed response-byte, and decoded-event caps; Azuro history is account-scoped to a validated bettor wallet and excludes multi-selection bets.
...
Omen and Gnosis now expose bounded, wallet-filtered FPMM creator activity from the official subgraph and turn validated rows into simulation-first paper-copy previews. Activity remains finite and non-complete by design; live copy submission, collateral approval, settlement, and CLOB depth are not claimed.

Thales Market's public AMM reads and paper-order previews remain implemented. Live submission is now fail-closed until signed calldata can be decoded and bound to the reviewed network, AMM target, market, position, size, and price; the former metadata-only forwarding boundary is superseded.
...
slice and may return fewer rows or omit the earliest partial candle bucket; every
retained candle from such a truncated scan carries `partial=true` and
`history_coverage=bounded_slot_slice`. Timestamp-complete requests must reach
pair creation and verify the declared creation swap within the configured scan
horizon or fail closed; arbitrary full-history completeness is not claimed. Wallet
activity filters the documented `maker` field across a bounded recent ticker/trade
...
| Interactive Brokers ForecastTrader / IBKR Prediction Markets (`ibkr_forecasttrader`) | Implemented | Yes | Yes (executions/OHLC/account order reads) | Yes | Guarded, off by default (cancel/cancel-all also guarded) | Yes, simulation-first account executions | Required | IBKR account required | Region/KYC limited |
| ForecastEx (`forecastex`) | Implemented | Yes | Yes (executions/OHLC/account order reads) | Yes | Guarded, off by default (cancel/cancel-all also guarded) | Yes, simulation-first account executions | Required | IBKR account required | Region/KYC limited |
| CME Group Prediction Markets (`cme_prediction_markets`) | Implemented | Yes | Yes (executions/OHLC/account order reads) | Yes | Guarded, off by default (cancel/cancel-all also guarded) | Yes, simulation-first account executions | Required | IBKR account required | Region/KYC limited |
...
| Hyperliquid (`hyperliquid`) | Implemented | Yes | Yes (HIP-4 wallet fills/candles) | Yes | Guarded, off by default; signed cancel/cancel-by-cloid/schedule-cancel also guarded | Yes (HIP-4 wallet fills; simulation-first) | Required | No API key for reads; externally signed wallet payload required for live orders | Jurisdiction varies |
| Myriad Markets (`myriad_markets`) | Implemented | Yes | Yes (trades/candles/account activity/portfolio/market positions) | Yes | Guarded, off by default; cancel/batch-cancel/cancel-all also guarded | Yes, simulation-first | Required | API credentials required | Jurisdiction varies |
...
| Gemini Titan / Gemini Predictions (`gemini_titan`) | Implemented | Yes | Yes (filled account trades/price-history points) | Yes | Guarded, off by default (orders and single/batch cancellation) | Yes, simulation-first filled orders | Required | Live trading only | Region/KYC limited |
...
| Thales Market (`thales_market`) | Implemented | Yes | Yes | Yes | No (signed-calldata decoder pending) | No | Required | Not required | Jurisdiction varies |
| Hedgehog Markets (`hedgehog_markets`) | Implemented | Yes | Yes | Yes | No (signed-instruction decoder pending) | No | Required | Not required | Jurisdiction varies |
| Omen (`omen`) | Implemented | Yes | Yes (public FPMM trades; bounded derived candles; creator activity) | Yes | No (signed-calldata decoder pending) | Yes, simulation-first (bounded creator activity) | Required | Subgraph endpoint required | Jurisdiction varies |
| Zeitgeist (`zeitgeist`) | Implemented | Yes | Yes | Yes | No (signed-extrinsic decoder pending) | No | Required | Not required | Jurisdiction varies |
| Zeitgeist SDK / Markets (`zeitgeist_sdk_markets`) | Implemented | Yes | Yes | Yes | No (signed-extrinsic decoder pending) | No | Required | Not required | Jurisdiction varies |
| Azuro (`azuro`) | Implemented | Yes (account-scoped single-selection bets and derived candles) | Yes | Yes | No (signed-clientBetData decoder pending) | Yes (simulation-first single-bet previews) | Required | Not required | Jurisdiction varies |
| SX Bet / SX Network (`sx_bet`) | Implemented | Yes | Yes | Yes (public trades plus bounded derived trade-tape candles and API-key account bets/fills/positions) | Guarded, off by default (v3 order/event/all cancellation) | Yes, simulation-first authenticated fills | Required | API credentials required | Jurisdiction varies |
...
| Smarkets (`smarkets`) | Implemented | Yes | Yes (authenticated orders/account, executed trades, bounded derived candles) | Yes | Guarded place/cancel, off by default | Yes, simulation-first (complete authenticated executed orders) | Required | Exchange account/API keys | Region/KYC limited |
| Betfair Exchange (`betfair_exchange`) | Implemented | Yes | Yes (current/cleared account orders, bounded derived execution candles, funds, account details, statements, currency rates) | Yes | Guarded place/cancel, off by default | Yes, simulation-first | Required | Exchange account/API keys | Region/KYC limited |
...
| Seer (`seer`) | Implemented | Yes | Yes (official Generic-market DEX-pool chart points) | Yes | No (signed-calldata decoder pending) | No | Required | Not required | Jurisdiction varies |
| DFlow (`dflow`) | Implemented | Yes | Yes (public trades plus bounded derived trade-feed candles) | Yes | No (signed-instruction decoder pending) | Yes, simulation-first (wallet-filtered on-chain fills) | Required | API key required | Region limited |
...
| Xmarket (`xmarket`) | Implemented | Yes (positions/orders; account fills) | Yes | Yes | Guarded, off by default (single create; batch cancel) | Yes (authenticated-fill simulation previews) | Required | API credentials required | Jurisdiction varies |
| Trueo (`trueo`) | Implemented | Yes | Yes (V3/V4 prices, bounded public swaps, and derived candles) | Yes | No (router-calldata decoder pending) | No | Required | Not required | Jurisdiction varies |
...
| Gnosis Prediction Markets (`gnosis_prediction_markets`) | Implemented | Yes | Yes (public FPMM trades; bounded derived candles; creator activity) | Yes | No (signed-calldata decoder pending) | Yes, simulation-first (bounded creator activity) | Required | Subgraph endpoint required | Jurisdiction varies |
| MetaDAO (`metadao`) | Implemented | Yes | Yes (one-level bid/ask top-of-book; bounded recent swaps; derived candles) | Yes | No (signed-instruction decoder pending) | Yes, simulation-first (bounded maker activity) | Required | Not required | Jurisdiction varies |
...
| Lamas Finance (`lamas_finance`) | Implemented | Yes | Yes | Yes | No (signed-instruction decoder pending) | No | Required | Not required | Devnet example; production deployment must be reviewed |
...
| Zeitgeist Prediction Pools (`zeitgeist_prediction_pools`) | Implemented | Yes | Yes | Yes | No (signed-extrinsic decoder pending) | No | Required | Not required | Jurisdiction varies |
...
| Prophet Exchange (`prophet_exchange`) | Implemented | Yes | Yes (authenticated filled trades, bounded derived candles, and simulation-first copy previews) | Yes | Guarded, off by default | Yes (authenticated fills, simulation only) | Required | API credentials required | Region/KYC limited |
...
| Matchbook (`matchbook`) | Implemented | Yes | Yes | Yes (matched bets plus bounded derived candles, settled/current bets) | Guarded, off by default; cancel-only offer mutations are separately opt-in | Yes, simulation-first | Required | Exchange account/API keys | Region/KYC limited |
| Meta Arena (`meta_arena`) | Verified blocked | No | No | No | No | No | Required | Account required | Jurisdiction varies |

Context V2 copy activity uses the official SDK's read-only `orders.list` filter (`trader`, `status=filled`, bounded `limit`). Only complete binary orders with matching wallet, BUY/SELL direction, 1e6-scaled filled size, probability price, nonce, and timestamp become simulation-first copy previews; no activity poll submits a live order. Signed single and batch cancellation uses the official fixed `/orders/cancel` and `/orders/bulk/cancel` paths and remains disabled unless the Context opt-in, shared live gates, exact confirmation, and external wallet signature are present.
...
- **Wallets & Copy** manages tracked identities for markets with an official activity feed (Polymarket, Opinion Labs, Manifold, Myriad, Hyperliquid HIP-4, Predict.fun, Probable, Azuro, Context V2, MetaDAO, and Omen/Gnosis). Polymarket also supports username/profile search; Manifold requires `manifold:<username>`; MetaDAO requires `solana:<base58 wallet>`; and the other feeds require a canonical 0x wallet address. Each feed is bounded by its documented retention/limit contract and remains simulation-first.
