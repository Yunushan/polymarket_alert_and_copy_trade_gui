...
def _validated_market_ws_url(url_base: str, *, resolve_addresses: bool = False) -> str:
    ...
    from market_adapters.outbound import OutboundEndpointPolicy, validate_outbound_url
...
def build_market_subscription(
    ...
    *,
    ...
    msg: Dict[str, Any] = {
        "assets_ids": [str(token_id) for token_id in token_ids if token_id],
        "type": "market",
    }
...
class MarketWSClient:
    """Synchronous-loop market-channel client for Polymarket CLOB events."""
    ...
    def __init__(
        ...
        *,
        ...
        self._token_ids: Set[str] = {str(token_id) for token_id in token_ids if token_id}
        self._state_lock = threading.RLock()
        self._on_event = on_event
        self._custom_feature_enabled = custom_feature_enabled
        self._verbose = verbose
        self._url = _validated_market_ws_url(url_base)
    ...
    def set_tokens(self, token_ids: Iterable[str]) -> None:
        with self._state_lock:
            self._token_ids = {str(token_id) for token_id in token_ids if token_id}
    ...
    def subscribe(self, token_ids: Iterable[str]) -> None:
        ...
        if not normalized:
            return
        with self._state_lock:
            self._token_ids.update(normalized)
    ...
    def unsubscribe(self, token_ids: Iterable[str]) -> None:
        ...
        if not normalized:
            return
        with self._state_lock:
            self._token_ids.difference_update(normalized)

    def _token_snapshot(self) -> Set[str]:
        with self._state_lock:
    ...
    def _connect_once(
        ...
        try:
            ...
            sent_tokens = self._token_snapshot()
            connection.send(
                json.dumps(
            ...
            while not stop_event.is_set():
                sent_tokens = self._sync_subscriptions(connection, sent_tokens)
                now = time.monotonic()
                if now >= next_ping_at:
    ...
    def _sync_subscriptions(self, connection: Any, sent_tokens: Set[str]) -> Set[str]:
        current_tokens = self._token_snapshot()
        removed = sorted(sent_tokens - current_tokens)
        if removed:
            ...
            sent_tokens.difference_update(removed)

        added = sorted(current_tokens - sent_tokens)
        if added:
            message = {"assets_ids": added, "operation": "subscribe"}
            if self._custom_feature_enabled:
                message["custom_feature_enabled"] = True
            connection.send(json.dumps(message))
            sent_tokens.update(added)
        return sent_tokens
    ...
    def _handle_message(self, message: Any) -> None:
        ...
        try:
            ...
            if self._verbose:
                print("[ws] non-json:", str(message)[:200])
