...
class VerificationFixtureTests(unittest.TestCase):
    ...
    def test_sx_bet_fixtures_cover_core_payload_shapes(self) -> None:
        ...
        orders_v3 = json.loads((FIXTURE_ROOT / "sx_bet" / "orders_v3.json").read_text(encoding="utf-8"))
        order_detail_v3 = json.loads((FIXTURE_ROOT / "sx_bet" / "order_detail_v3.json").read_text(encoding="utf-8"))
        trades_v3 = json.loads((FIXTURE_ROOT / "sx_bet" / "trades_v3.json").read_text(encoding="utf-8"))
        fills_v3 = json.loads((FIXTURE_ROOT / "sx_bet" / "fills_v3.json").read_text(encoding="utf-8"))
        positions_v3 = json.loads((FIXTURE_ROOT / "sx_bet" / "positions_v3.json").read_text(encoding="utf-8"))
        balance_v3 = json.loads((FIXTURE_ROOT / "sx_bet" / "balance_v3.json").read_text(encoding="utf-8"))
        cancel_v3 = json.loads((FIXTURE_ROOT / "sx_bet" / "cancel_v3.json").read_text(encoding="utf-8"))
        ...
        self.assertIsInstance(orders_v3.get("data", {}).get("orders"), list)
        self.assertIn("remainingSize", orders_v3["data"]["orders"][0])
        self.assertIn("order", order_detail_v3.get("data", {}))
        self.assertIsInstance(trades_v3.get("data", {}).get("trades"), list)
        self.assertIn("ownOrderIds", trades_v3["data"]["trades"][0])
        self.assertIsInstance(fills_v3.get("data", {}).get("fills"), list)
        self.assertIn("matchId", fills_v3["data"]["fills"][0])
        self.assertIsInstance(positions_v3.get("data", {}).get("positions"), list)
        self.assertIn("maxLoss", positions_v3["data"]["positions"][0])
        self.assertIsInstance(balance_v3.get("data", {}).get("balances"), list)
        self.assertIn("availableAmount", balance_v3["data"]["balances"][0])
        self.assertIsInstance(cancel_v3.get("data", {}).get("cancelled"), list)
    ...
    def test_additional_official_adapter_fixtures_cover_core_payload_shapes(self) -> None:
        ...
        xo_trades = json.loads((FIXTURE_ROOT / "xo_market" / "trades.json").read_text(encoding="utf-8"))
        xo_candles = json.loads((FIXTURE_ROOT / "xo_market" / "candles.json").read_text(encoding="utf-8"))
        xo_account = json.loads((FIXTURE_ROOT / "xo_market" / "account.json").read_text(encoding="utf-8"))
        xo_positions = json.loads((FIXTURE_ROOT / "xo_market" / "positions.json").read_text(encoding="utf-8"))
        xo_orders = json.loads((FIXTURE_ROOT / "xo_market" / "orders.json").read_text(encoding="utf-8"))
        xo_account_trades = json.loads(
            (FIXTURE_ROOT / "xo_market" / "account_trades.json").read_text(encoding="utf-8")
        )
        xo_settlement = json.loads((FIXTURE_ROOT / "xo_market" / "settlement.json").read_text(encoding="utf-8"))
        xo_settlement_history = json.loads(
            (FIXTURE_ROOT / "xo_market" / "settlement_history.json").read_text(encoding="utf-8")
        )
        xo_audit_logs = json.loads((FIXTURE_ROOT / "xo_market" / "audit_logs.json").read_text(encoding="utf-8"))
        ...
        self.assertIsInstance(xo_trades.get("trades"), list)
        self.assertIn("trade_id", xo_trades["trades"][0])
        self.assertIsInstance(xo_candles.get("candles"), list)
        self.assertIn("open", xo_candles["candles"][0])
        self.assertEqual(xo_account.get("status"), "active")
        self.assertIsInstance(xo_positions.get("positions"), list)
        self.assertIn("outcome_id", xo_positions["positions"][0])
        self.assertIsInstance(xo_orders.get("orders"), list)
        self.assertIn("status", xo_orders["orders"][0])
        self.assertIsInstance(xo_account_trades.get("trades"), list)
        self.assertIn("trade_id", xo_account_trades["trades"][0])
        self.assertEqual(xo_settlement.get("status"), "resolved")
        self.assertIsInstance(xo_settlement_history.get("settlements"), list)
        self.assertIsInstance(xo_audit_logs.get("events"), list)
        self.assertIn("event_type", xo_audit_logs["events"][0])
